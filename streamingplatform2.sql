...
SELECT u.user_id, u.user_name, COUNT(l.user_id) AS total_listens 
FROM users u 
LEFT JOIN listens l ON u.user_id = l.user_id
LEFT JOIN tracks t ON l.track_id = l.track_id AND t.genre = 'Pop';
...
JOIN tracks t ON l.track_id = t.track_id AND t.genre = 'Rock'
GROUP BY u.user_id 
HAVING SUM(CASE WHEN t.genre != 'Rock' THEN 1 ELSE 0 END) = 0;
...
SELECT DISTINCT artist_name, total_revenue
FROM (
	SELECT artist_name, total_revenue, AVG(total_revenue) AS avg_revenue
	FROM (
		SELECT a.artist_name, COUNT(l.user_id) OVER(PARTITION BY a.artist_id) AS total_revenue
		FROM artists a 
		JOIN tracks t ON a.artist_id = t.artist_id
...
FROM (
	SELECT user_id, users_dur_lis,
	  LAG(users_dur_lis) OVER(PARTITION BY user_id ORDER BY listen_date) AS prev_users_dur_lis
	FROM (
		SELECT l.user_id, l.listen_date, 
		  SUM(t.duration_sec) OVER(PARTITION BY l.user_id ORDER BY l.listen_date) AS users_dur_lis,
		FROM listens l 
		JOIN tracks t ON l.track_id = t.track_id
	) t2 
	GROUP BY user_id
	HAVING SUM(CASE WHEN users_dur_lis < prev_users_dur_lis THEN 1 ELSE 0 END) = 0
) t;	
...
FROM (
	...
	WHERE users_dur_lis > prev_users_dur_lis
) t;	
...
SELECT DISTINCT user_id 
FROM (
	SELECT user_id, COUNT(DISTINCT listen_date) AS cnt
	FROM listens 
	GROUP BY user_id
	HAVING COUNT(DISTINCT listen_date) = 5
) t;
...
JOIN subscriptions s2 ON s1.end_date <= s2.start_date
...
SELECT DISTINCT user_id, CASE 
	WHEN cnt4 >= 10 THEN 'BINGE'
	WHEN cnt5 >= 10 THEN 'CASUAL'
	ELSE 'INACTIVE'
  END AS users_category 
FROM (
	SELECT user_id, 
	  COUNT(user_id) OVER(PARTITION BY listen_date) AS cnt4,
	  COUNT(user_id) OVER() AS cnt5 
	FROM listens 
) t;
